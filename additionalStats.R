...
getCellsGraph <- function(spp, stats, dispersalApplied) {
  # GRAPH WITH NUMBER OF CELLS OF ALL TIME PERIODS
...
getUrbProtGraph <- function(spp, stats, dispersalApplied) {
  ...
  if (ProtectedAnalysis == "Y") {
    ...
    PercentProtected <- unlist(rapply(list(PercentProtected), f = function(x) ifelse(is.nan(x), 0, x), how = "replace"))
  } else {
  ...
  if (UrbanAnalysis == "Y") {
    ...
    PercentUrbanized <- unlist(rapply(list(PercentUrbanized), f = function(x) ifelse(is.nan(x), 0, x), how = "replace"))
  } else {
...
#Creates a bar graph comparing area of suitable habitat between dispersal-constrained analysis and regular analysis
DispersalCompare <- function(spp, stats, myvars) {
  ...
  for(scen in 1:numScenario) {
    ...
    pdf(file = paste0(result_dir, "/", spp, "/Dispersal Applied Additional Stats/", Scenarios[scen], "_DispersalCompare.pdf"))
