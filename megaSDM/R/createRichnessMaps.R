...
#' create richness maps for a list of species. If higher taxa are provided \code{taxonlist != FALSE},
#' separate richness for each higher taxon will be created in addition to the
#' full species richness maps. Given hindcasted/forecasted binary maps, future/past
#' species richness will also be calculated. Rasters and .pdf files of each are
#' generated, and the legend is standardizes across all .pdf maps for effective
#' comparisons. Finally, provided distribution maps that are constrained by dispersal
#' rate, this function compares between the dispersal-constrained and regular richness
#' maps.
#'
...
#' @param time_periods a vector of the years in which the projection will occurwith the first element
...
#' needed? If dispersal rate. If these analysis are not needed, or if the \code{megaSDM::dispersalRate}
#' function has yet to be run, this should be set to \code{FALSE} (the default).
...
#' a list of all species modelled (Column 1) and a selected high taxon
...
createRichnessMaps <- function(result_dir, time_periods, scenarios = NA,
  ...
  for (sp in 1:nrow(ListSpp)) {
    ...
    curfocus <- list.files(path = getwd(), pattern = paste0("binary.bil$"))
  ...
  FutureRichnessMaps <- function(taxon, y, s, disp) {
    ...
    raster::writeRaster(FutureRichness,
                filename = paste0("Richness_", taxon, "_", FocusYear, "_", FocusScenario, filepath),
                format = "EHdr",
                overwrite = TRUE,
                prj = TRUE)
  }
  ...
  DispersalDiff <- function(taxon, y, s) {
    ...
    DispersalSR <- raster::raster(paste0("Richness_", taxon, "_", FocusYear, "_", FocusScenario, "_dispersalRate.bil"))
    ...
    raster::writeRaster(DifferenceSR,
                filename = paste0("Richness Difference_", taxon, "_", FocusYear, "_", FocusScenario, ".bil"),
                format = "EHdr",
                overwrite = TRUE,
                prj = TRUE)
  ...
  MakePDFMaps <- function(taxon) {
    ...
    setwd(RichnessMaps)
    RasterList <- list.files(path = getwd(), pattern = paste0("\\.bil$"))
  ...
  for (t in 1:ntaxa) {
    ...
    raster::writeRaster(CurrentRichness,
                filename = paste0("Richness_", taxon, "_", time_periods[1], ".bil"),
                format = "EHdr",
                overwrite = TRUE,
                prj = TRUE)
    ...
    if (dispersal == TRUE) {
      ...
      for (sp in 1:length(taxonlist2)) {
        ...
        curfocus <- list.files(path = getwd(), pattern = paste0("binary_dispersalRate.bil$"))
        if (!length(curfocus) > 0) {
