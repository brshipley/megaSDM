...
If this is your first time running MaxEnt, double-click the "maxent.jar" file to ensure that it can open. If the maxent.jar dialog box does not load when opened (this is common when using MacOS), the Java Runtime Environment may need to be downloaded before running megaSDM (https://www.oracle.com/java/technologies/javase-jre8-downloads.html).

Finally, set your working directory to a location where you want all of outputs megaSDM provides to be written out, and *place the maxent.jar file in it*
This is necessary so that the jar file can properly interface with the computer.
